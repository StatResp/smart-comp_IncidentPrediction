    ...
    def fit(self, df, metadata=None,sampling_name='No_Resample'):
        ...
        for temp_cluster in clusters:
            ...
            if (self.model_name == 'Poisson_Regression'):
                ...
                mask=y_train.index.isin(y_train.sample(int(split_point*len(y_train)),replace=False).index)    
                                
                model = sm.GLM(y_train[mask], x_train[mask], family=sm.families.Binomial(sm.families.links.logit())).fit(method="lbfgs")  #lbfgs #bfgs irls
                y_train_hat= model.predict(x_train[~mask])
                precision, recall, thresholds = precision_recall_curve(y_train[~mask], y_train_hat)
                ...
                print('===================')
                #model = sm.ZeroInflatedPoisson(endog=y_train, exog=x_train, exog_infl=x_train, inflation='logit').fit(method="bfgs",maxiter=500,full_output=True,disp=True)  #method="nm" method='bfgs' lbfgs    #https://www.statsmodels.org/stable/generated/statsmodels.discrete.count_model.ZeroInflatedPoisson.fit.html
