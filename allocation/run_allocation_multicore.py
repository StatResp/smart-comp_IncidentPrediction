...
def get_dist_metric_for_allocation(arguments):
    '''

    '''

    num_resources, alpha, model_i, DF_Test_spacetime, All_seg_incident, Grid_center, df_incident, Delay, Penalty, Window_size_hour, possible_facility_locations, demand_nodes, Distant_Dic, time_range = arguments
    ...
    Figure_Tag=False
    for row,row_values  in time_range.iloc[0:2].iteritems():  #row,row_values  in time_range.iteritems()
    ...
    # DF_metric.mean().to_json('output/Distance/test_{}_{}_{}.json'.format(num_resources, alpha, model_i),orient='table')
...
if __name__ == "__main__":
    ...
    DF_Test_spacetime=pd.read_pickle('output/DF_Test_spacetime_All.pkl')     #pd.read_pickle('D:/inrix/prediction_engine_20/output/DF_Test_spacetime_AllMethods_None_Alltestwindow.pkl')
    ...
    alpha_list=[0,1]#[0.0,0.5,1.0,2.0]
    model_list= ['naive','LR+NoR+NoC1','RF+CW+NoC1'] # ['RF+CW+NoC1', 'RF+NoR+NoC1', 'RF+RUS+NoC1','RF+ROS+NoC1','RF+CW+KM2', 'RF+NoR+KM2', 'RF+RUS+KM2','RF+ROS+KM2']
    ...
    with Pool(processes=7) as pool:
        res_dict = pool.map(get_dist_metric_for_allocation, experimental_inputs)

    # for args in experimental_inputs:
    #     get_dist_metric_for_allocation(args)


    print('compu time: {}'.format(time.time() - start_time))
...
#     df_responders_Exist=False
#     Figure_Tag=False
#     for row,row_values  in time_range.iloc[0:1].iteritems():
#         if (model_i!='naive') | (df_responders_Exist==False):
#             print(row,row_values)
#             weights_dict, DF_Test_space_time_i=Weight_and_Merge(DF_Test_spacetime,All_seg_incident,time_i=row_values,model=model_i)
#             allocator = pmedianAllocator()
